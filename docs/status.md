...
3 Dependabot dependency-bump PRs are now past the 24h stalled threshold — #1/#2 (~14 days) and #5 (dashboard group, ~33h, crossed the threshold since yesterday). No `auto-merge` skill is enabled, so they sit until reviewed. `heartbeat` is the only enabled skill and it is running now; its own entry is self-referential and never counts toward its own verdict. It last succeeded at 09:28 UTC yesterday (`consecutive_failures` 0) — the 80% lifetime success rate is a bootstrap artifact from early warming-up runs.
...
|-------|----------|--------|-------------:|---------------------:|
| heartbeat | 2026-08-01 09:28 UTC | ✅ success | 80% | 0 |
