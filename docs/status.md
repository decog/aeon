...
`heartbeat` is the only enabled skill and it is running now; its own entry is self-referential and never counts toward its own verdict. It last succeeded at 08:13 UTC today (`consecutive_failures` 0) — the low lifetime success rate is a bootstrap artifact from early warming-up runs.
...
|-------|----------|--------|-------------:|---------------------:|
| heartbeat | 2026-07-19 08:13 UTC | ⏳ dispatched | 33% | 0 |
