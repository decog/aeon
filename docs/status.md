...
**Overall:** 🟡 WATCH
**Updated:** 2026-07-26 09:06 UTC
**Open issues:** 0
**Next scheduled run:** heartbeat at 08:00 UTC

3 Dependabot dependency-bump PRs (#1/#2/#3) have been open ~7 days, past the 24h stalled threshold — no `auto-merge` skill is enabled, so they sit until reviewed. `heartbeat` is the only enabled skill and it is running now; its own entry is self-referential and never counts toward its own verdict. It last succeeded at 08:53 UTC yesterday (`consecutive_failures` 0) — the 69% lifetime success rate is a bootstrap artifact from early warming-up runs.
...
|-------|----------|--------|-------------:|---------------------:|
| heartbeat | 2026-07-25 08:53 UTC | ✅ success | 69% | 0 |
