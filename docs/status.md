...
3 Dependabot dependency-bump PRs (#1/#2/#3) have been open ~6 days, past the 24h stalled threshold — no `auto-merge` skill is enabled, so they sit until reviewed. `heartbeat` is the only enabled skill and it is running now; its own entry is self-referential and never counts toward its own verdict. It last succeeded at 09:39 UTC yesterday (`consecutive_failures` 0) — the 67% lifetime success rate is a bootstrap artifact from early warming-up runs.
...
|-------|----------|--------|-------------:|---------------------:|
| heartbeat | 2026-07-24 09:39 UTC | ✅ success | 67% | 0 |
